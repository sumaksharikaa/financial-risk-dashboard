...
@st.cache_data
def load_data():
    engine = create_engine('postgresql+psycopg2://localhost/lending_risk')
    query = """
        SELECT l.grade, l.loan_amount, l.int_rate, l.dti,
               l.fico_range_low, l.purpose, l.term,
               l.loan_status, l.default_flag,
               l.issue_date, b.annual_income,
               b.home_ownership, b.addr_state
        FROM loans l
...
with col2:
    ...
    df_filtered2['fico_band'] = pd.cut(
        df_filtered2['fico_range_low'],
        bins=[0,649,699,749,799,850],
        labels=['<650','650-699','700-749','750-799','800+']
    )
    fico_stats = df_filtered2.groupby('fico_band')['default_flag'].mean() * 100
    fig, ax = plt.subplots(figsize=(7, 5))
...
df_filtered['issue_date'] = pd.to_datetime(df_filtered['issue_date'])
...
ax1.fill_between(range(len(monthly)), monthly['volume']/1e6,
...
ax2.plot(range(len(monthly)), monthly['default_rate']*100,
         color='crimson', lw=2, linestyle='--', label='Default Rate %')
